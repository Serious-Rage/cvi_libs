...
var cvi_sphere = {
    ...
    add: function(image, options) {
        if (image.tagName.toUpperCase() == "IMG") {
            ...
            var size = (typeof options['size'] == 'number' ? options['size'] : cvi_sphere.defaultSize);
            if (size <= 0) {
                size = Math.max(32, (nw > nh ? image.height : nh > nw ? image.width : image.height));
            }
            if (size < 32) {
                size = 32;
            }
            try {
                ...
                if (document.all && document.namespaces && !window.opera && (!document.documentMode || document.documentMode < 9)) {
                    ...
                    var canvas = document.createElement(['<var style="zoom:1;overflow:hidden;display:' + display + ';width:' + size + 'px;height:' + size + 'px;padding:0;">'].join(''));
    ...
    modify: function(canvas, options) {
        function drawEllipse(ctx, x1, y1, x2, y2) {
            ...
            ctx.beginPath();
            ctx.moveTo(cx, cy - ry);
            ctx.bezierCurveTo(cx + (kp * rx), cy - ry, cx + rx, cy - (kp * ry), cx + rx, cy);
            ctx.bezierCurveTo(cx + rx, cy + (kp * ry), cx + (kp * rx), cy + ry, cx, cy + ry);
            ctx.bezierCurveTo(cx - (kp * rx), cy + ry, cx - rx, cy + (kp * ry), cx - rx, cy);
            ctx.bezierCurveTo(cx - rx, cy - (kp * ry), cx - (kp * rx), cy - ry, cx, cy - ry);
            ctx.closePath();
            return false;
        ...
        function hex2rgb(val, f, isIE) {
            ...
            if (isIE) {
                ...
                return Math.floor(cr * f) + ',' + Math.floor(cg * f) + ',' + Math.floor(cb * f);
            }
        };
        try {
            var alpha = (typeof options['alpha'] == 'number' ? options['alpha'] : canvas.options['alpha']);
            canvas.options['alpha'] = alpha;
            var shade = (typeof options['shade'] == 'number' ? options['shade'] : canvas.options['shade']);
            canvas.options['shade'] = shade;
            var shine = (typeof options['shine'] == 'number' ? options['shine'] : canvas.options['shine']);
            canvas.options['shine'] = shine;
            var zoom = (typeof options['zoom'] == 'number' ? options['zoom'] : canvas.options['zoom']);
            canvas.options['zoom'] = zoom;
            var shift = (typeof options['shift'] == 'number' ? options['shift'] : canvas.options['shift']);
            canvas.options['shift'] = shift;
            var filter = (typeof options['filter'] == 'object' ? options['filter'] : canvas.options['filter']);
            canvas.options['filter'] = filter;
            var callback = (typeof options['callback'] == 'string' ? options['callback'] : canvas.options['callback']);
            canvas.options['callback'] = callback;
            var color = (typeof options['color'] == 'string' ? options['color'] : canvas.options['color']);
            canvas.options['color'] = color;
            var color2 = (typeof options['color2'] == 'string' ? options['color2'] : canvas.options['color2']);
            canvas.options['color2'] = color2;
            var c1 = 0;
            if (isNaN(color)) c1 = (color.match(/^#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]$/i) ? color : 0);
            var c2 = 0;
            if (isNaN(color2)) c2 = (color2.match(/^#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]$/i) ? color2 : 0);
            var sz = canvas.width,
            ...
            if (document.all && document.namespaces && !window.opera && (!document.documentMode || document.documentMode < 9)) {
                var head, foot, shadow, border, shades, shines, frame, fill = '';
                ...
                if (canvas.tagName.toUpperCase() == "VAR") {
                    ...
                    foot = '</v:group>';
                    if (isNaN(c1)) {
                        fill = '<v:oval filled="t" stroked="f" strokeweight="0" style="zoom:1;margin:0;padding:0;display:block;position:absolute;top:1px;left:' + os + 'px;width:' + iw + 'px;height:' + ih + 'px;"><v:fill color2="' + c1 + '" color="' + (isNaN(c2) ? c2 : c1) + '" type="gradient" method="sigma" angle="20" focus="0" focussize="0,0" focusposition="1,1" on="t" /></v:oval>';
                    }
                    ...
                    img.onload = function() {
                        if (prepared && (typeof cvi_filter != 'undefined') && filter != null && filter.length > 0) {
                            ...
                            if (src && ctx) {
                                ...
                                src.drawImage(img, Math.round(nx + (nw * (zo * xo))), Math.round(ny + (nh * (zo * yo))), Math.round(nw * (1 - zo)), Math.round(nh * (1 - zo)), 0, 0, iw + 4, ih + 4);
                                src.drawImage(img, Math.round(nx + (nw * (zo * xo))), Math.round(ny + (nh * (zo * yo))), Math.round(nw * (1 - zo)), Math.round(nh * (1 - zo)), 2, 2, iw, ih);
                        ...
                        st.addColorStop(.8, 'rgba(' + gc + ',' + (sh * .125) + ')');
                        ...
                        st.addColorStop(1, 'rgba(0,0,0,' + (sh * .5) + ')');
                        context.fillStyle = st;
                        if (canvas.isWK) {
                            context.fill();
                        } else {
